...
namespace Web.Manager.WebManager.Entity
{
    ...
    public class EnumAttribute : Attribute
    {
        ...
        public EnumAttribute(string name, string description, string cssname)
        {
            this.Name = name;
            this.Description = description;
    ...
    public class EnumAutoHelper
    {
        ...
        /// <param name="value">枚举值</param>
        /// <returns>枚举值对应的名称</returns>
        public static string GetCssName(Enum value)
        {
            EnumAttribute ea = GetAttribute(value);
            return ea != null ? ea.CssName : "";
        }
        ...
        /// <param name="enumType"></param>
        /// <returns></returns>
        public static Dictionary<string, EnumJsItem> GetJsItem(Type enumType)
        {
            Type underlyingType = Enum.GetUnderlyingType(enumType);
            Dictionary<string, EnumJsItem> dic = new Dictionary<string, EnumJsItem>();
            foreach (object o in Enum.GetValues(enumType))
            {
                Enum e = (Enum)o;
                string value = Convert.ChangeType(o, underlyingType).ToString();
                var addItem = new EnumJsItem()
                {
                    cssname = GetCssName(e),
                    des = GetDescription(e),
                    id = value,
                    name = GetName(e)
                };
                dic.Add(value, addItem);
            }
            return dic;
        }
