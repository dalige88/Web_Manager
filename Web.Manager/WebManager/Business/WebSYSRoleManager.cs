...
{
    public class WebSYSRoleManager : Web.Manager.WebManager.Business.ServiceBase
    {
        public WebSYSRoleManager(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }

        public AjaxResult<List<WebSysRole>> GetAllRole()
        {
            var Roles = db.WebSysRole.ToList();
            return new AjaxResult<List<WebSysRole>>(Roles);
        }
        ...
        /// <returns></returns>
        public AjaxResult<RoleInfo> GetRoleInfo(int RoleID)
        {
            ...
            return new AjaxResult<RoleInfo>(info);
        }


        public AjaxResult<Object> SaveRole(RoleInfo info)
        {
            if (info == null)
                return new AjaxResult<Object>("提交数据无效！");
            if (string.IsNullOrEmpty(info.RoleName))
                return new AjaxResult<Object>("请输入角色名称！");
            if (info.RoleMenus.Count == 0)
                return new AjaxResult<Object>("请选择管理权限！");
