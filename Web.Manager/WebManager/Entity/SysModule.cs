...
namespace Web.Manager.WebManager.Entity
{
    public class SysModule
    {
        public static List<FirstMenu> InitPermission()
        {
            ...
            var types = Assembly.Load(ss.Substring(0, index))
                .GetTypes()
                .Where(e => e.BaseType.Name == nameof(BaseController));
            foreach (var type in types)
            {
                var members = type.GetMethods()
                    .Where(e => (e.ReturnType.BaseType != null && e.ReturnType.BaseType.Name == nameof(IActionResult)) || e.ReturnType.Name == nameof(ActionResult)).ToList();
                //二级菜单
