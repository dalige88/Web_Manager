...
namespace DB.SQLITE
{
    ...
    public class SqlLiteAdoSession : IDisposable
    {
        ...
        {
            string dbPathString = Environment.CurrentDirectory + @"/locdb/";
            if (!Directory.Exists(dbPathString))
            {
                Directory.CreateDirectory(dbPathString);
            }
            string pathFullString = string.Format("Data Source ={0}/locdb/{1}.db", Environment.CurrentDirectory, scheme.ToString().ToLower());
            //Console.WriteLine(pathFullString);
